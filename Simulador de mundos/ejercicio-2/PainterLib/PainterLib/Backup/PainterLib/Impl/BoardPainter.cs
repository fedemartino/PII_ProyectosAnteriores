...
namespace PainterLib
{
    ...
    /// </summary>    
    /// <remarks>
    /// La clase dibuja figuras dibujando líneas que unen los puntos de la 
    /// figura. Por ésto es necesario que la figura retorne sus puntos en 
    /// sentido horario o antihorario.   
    /// <br/>   
    /// Al momento de crearlo, es necesario proveer al pintor un tamanio de 
    /// celda. El tamanio de celda se utiliza en el metodo 
    /// <code>PaintInCell</code> para determinar el espacio a pintar.  
    /// <br/>    
    /// Por ejemplo, si la pizarra es de 1000x1000 y el tamanio de celda es 
    /// de 100, el pintor asumira que cuando le pidan pintar desde la celda 
    /// (0,0) a la (1,0) utilizando el metodo <code>PaintInBoard</code>, en 
    /// realidad tendra que pintar 100 puntos de la pizarra (el lado de una 
    /// celda).
    /// <br/>
    ...
    /// <code>PaintInCell</code>, dibujara un cuadrado de 80x80 puntos dentro 
    /// de la celda (1,1).
    /// </remarks>
    ...
    {       
        ...
        /// </summary>
        /// <param name="board">El <code>IBoard</code> donde mostrar la 
        /// figura.</param>
        public BoardPainter(IBoard board, Int32 cellSize)
        {
            Debug.Assert(board != null, "Board must not be null");
            Debug.Assert(cellSize > 0, "The cell size must be greater than 0");
        ...
        /// <summary>
        /// Dibuja la figura en la pizarra. Los puntos de la figura seran 
        /// adecuados en base al tamanio de la celda, de acuerdo a la 
        ...
        private void Paint(IShape shape, Color color)
        {
            ...
            lock (board)
            {
                for (Int32 i = 0; i < shape.Points.Length; i++)
                {
                    ...
                    Debug.Assert(from != null && to != null, "All the points " 
                        + "of the figure must not be null");
        ...
        private void Paint(IPoint point, Color color)
        {
            ...
            Debug.Assert(IsContainedInBoard(point), "point is not contained " 
                + "in board");
        ...
        {
            Debug.Assert(IsContainedInBoard(x, y), "point is not contained in " 
                + "board");
